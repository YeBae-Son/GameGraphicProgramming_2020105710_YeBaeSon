...
namespace library
{
    ...
    HRESULT VertexShader::Initialize(_In_ ID3D11Device* pDevice)
    {
        ...
        {
            MessageBox(
                nullptr, L"The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", 
                L"Error", 
                MB_OK
            );
            return hr;
        ...
        hr = pDevice->CreateInputLayout(layout, numElements, vsBlob->GetBufferPointer(), vsBlob->GetBufferSize(), m_vertexLayout.GetAddressOf());
